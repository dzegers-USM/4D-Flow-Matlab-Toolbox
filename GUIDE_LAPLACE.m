...
% Last Modified by GUIDE v2.5 05-Oct-2023 15:14:28
...
%     end
    ...
    for T = 1:number_of_time_slices-1; %Compute solution in-between time steps    
        ...
        %maskout = double(outlet_orig == outid);
        maskout = outlet_orig;
...
%     save('Output_vWERP/labels.mat','Labels')
    ...
    handles.normal_inlet = mean(handles.centerline(2:3,:) - handles.centerline(1:2,:))/norm(mean(handles.centerline(2:3,:) - handles.centerline(1:2,:)));
    handles.normal_outlet = mean(handles.centerline(end-2:end,:) - handles.centerline(end-3:end-1,:))/norm(mean(handles.centerline(end-2:end,:) - handles.centerline(end-3:end-1,:)));
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    if directory~=0
        ...
        save([directory,'\inlet.mat'],'inlet_seg','n_inlet', 'inlet','id_inlet')
        save([directory,'\outlet.mat'],'outlet_seg','n_outlet', 'outlet','id_outlet')
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % save excel with the curve

        VV = [{'DP_vWERP [mmHg]';'time [s]'}];
        MM = [[DP_VWERP(end), DP_VWERP(1:end-1)]*pa2mmhg;time];
        TT = table(VV,MM);
        TT.Properties.VariableNames = [{' ','CP'}];
 
        filename = [directory,'/vwerp_result.xlsx'];
        writetable(TT,fullfile(filename),'Sheet','vWERP','Range','A1')

        
        
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % guardamos la figura de pressure gradient
        pressure = [handles.DP_VWERP(end), handles.DP_VWERP(1:end-1)]*handles.pa2mmhg;
        [max_pressure, I1] = max(pressure);
        [min_pressure, I2] = min(pressure);
        max_pressure_time = handles.time(I1);
        min_pressure_time = handles.time(I2);

        f = figure;
        plot(handles.time, pressure,'b--*','LineWidth',2, 'MarkerSize', 8)
        hold on
        plot(handles.time, zeros(size(handles.time)),'k-')
        plot(max_pressure_time, max_pressure,'g*','MarkerSize', 8)
        plot(min_pressure_time, min_pressure,'r*','MarkerSize', 8)
        text(max_pressure_time + max_pressure_time*0.05, max_pressure,['\leftarrow ', num2str(max_pressure,4),' mmHg'])
        text(min_pressure_time + min_pressure_time*0.05, min_pressure,['\leftarrow ', num2str(min_pressure,5),' mmHg'])
        hold off
        xlabel('time [s]', 'Fontsize', 12,'FontWeight','bold')
        ylabel('Relative pressure drop [mmHg]', 'Fontsize', 12,'FontWeight','bold')
        grid on
        title(['Estimated relative pressure drops, WERP'], 'Fontsize', 14,'FontWeight','bold');
        deltay = [abs(min(pressure)), abs(max(pressure))];
        deltay = max(deltay);
        axis([handles.time(1), handles.time(end), min(pressure) - deltay*0.1, max(pressure) + deltay*0.1])
        dim = [.5 .5 .3 .3];
        str = ['Pulse Pressure, ',num2str(max(pressure) - min(pressure),4), ' mmHg'];
        annotation('textbox',dim,'String',str,'FitBoxToText','on');
        
        namefig = [directory,'/vwerp.png'];
        saveas(f,namefig)
        close(f)
        ...
        patch('faces',handles.faces,'vertices',handles.nodes,'facecolor','none','edgecolor',[0.5 0.5 0.5],'edgealpha',0.5)
        hold on
        plot3(handles.nodes(handles.id_inlet,1),handles.nodes(handles.id_inlet,2),handles.nodes(handles.id_inlet,3),'.b','markersize',7)
        center_in = mean(handles.nodes(handles.id_inlet,1:3));
        quiver3(center_in(1),center_in(2),center_in(3),handles.normal_inlet(1),handles.normal_inlet(2),handles.normal_inlet(3),30,'r','Linewidth',3)
        hold off
        axis vis3d
        ...
        patch('faces',handles.faces,'vertices',handles.nodes,'facecolor','none','edgecolor',[0.5 0.5 0.5],'edgealpha',0.5)
        hold on
        plot3(handles.nodes(handles.id_outlet,1),handles.nodes(handles.id_outlet,2),handles.nodes(handles.id_outlet,3),'.b','markersize',7)
        center_out = mean(handles.nodes(handles.id_outlet,1:3));
        quiver3(center_out(1),center_out(2),center_out(3),handles.normal_outlet(1),handles.normal_outlet(2),handles.normal_outlet(3),30,'r','Linewidth',3)
        hold off
        axis vis3d
...
% Dilate
    ...
    % handles.PLANE_INLET = double((handles.inlet_for_plot + handles.SEG_for_vwerp)==2);
    % handles.PLANE_OUTLET = double((handles.outlet_for_plot + handles.SEG_for_vwerp)==2);
    

    imlook3d(handles.PLANE_INLET)
    imlook3d(handles.PLANE_OUTLET)
    imlook3d(handles.SEG_for_vwerp)
    % disp(handles.normal_inlet)
    % disp(handles.normal_outlet)
...
% Erode
    ...
    handles.PLANE_INLET = double((handles.inlet_for_plot + handles.SEG_for_vwerp)==2);
    handles.PLANE_OUTLET = double((handles.outlet_for_plot + handles.SEG_for_vwerp)==2);
    handles.inlet_for_plot = handles.PLANE_INLET;
    handles.outlet_for_plot = handles.PLANE_OUTLET;
...
answer = questdlg('Do you want load the vWERP Folder, or a .csv file?', ...
	'vWERP Openning Question', ...
	'vWERP Folder','CSV File','vWERP Folder');
...
switch answer
    case 'vWERP Folder'
            ...
            inlet_var = load([folder_name,'/inlet.mat']);
            outlet_var = load([folder_name,'/outlet.mat']);
            mask_var = load([folder_name,'/mask.mat']);

            handles.inlet_for_plot = inlet_var.inlet;
            handles.outlet_for_plot = outlet_var.outlet;

            handles.normal_inlet = inlet_var.n_inlet;
            handles.normal_outlet = outlet_var.n_outlet;
            handles.n_inlet = inlet_var.n_inlet;
            handles.n_outlet = outlet_var.n_outlet;


            handles.id_inlet    = inlet_var.id_inlet;
            handles.inlet_seg   = inlet_var.inlet_seg;
            handles.inlet       = inlet_var.inlet;
            handles.id_outlet   = outlet_var.id_outlet;
            handles.outlet_seg  = outlet_var.outlet_seg;
            handles.outlet      = outlet_var.outlet;

            handles.PLANE_INLET = handles.inlet_seg;
            handles.PLANE_OUTLET = handles.outlet_seg;

            handles.mask = mask_var.mask;
    ...
    case 'CSV File'
        ...
        if handles.id_vwerp == 1
            ...
            for n=1:size(file,2)
                if contains(lower(file{n}),'inlet')==1
                    if contains(lower(file{n}),'flat')==1
                        T = readtable([path,file{n}]);
                        points_inlet = [T.Points_0, T.Points_1, T.Points_2]*1000;
                    else
                        T = readtable([path,file{n}]);
                        handles.id_inlet = T.IDNodes___;
                    end
                else 
                    if contains(lower(file{n}),'flat')==1
                        T = readtable([path,file{n}]);
                        points_outlet = [T.Points_0, T.Points_1, T.Points_2]*1000;
                    else
                        T = readtable([path,file{n}]);
                        handles.id_outlet = T.IDNodes___;
                    end
                end
            ...
            vector1 = points_inlet(1,:) - points_inlet(round(size(points_inlet,1)/2),:);
            vector2 = points_inlet(end,:) - points_inlet(round(size(points_inlet,1)/2),:);
            handles.normal_inlet = cross(vector1,vector2)/norm(cross(vector1,vector2));

            vector1 = points_outlet(1,:) - points_outlet(round(size(points_outlet,1)/2),:);
            vector2 = points_outlet(end,:) - points_outlet(round(size(points_outlet,1)/2),:);
            handles.normal_outlet = cross(vector1,vector2)/norm(cross(vector1,vector2));

            center_in = mean(points_inlet);
            center_out = mean(points_outlet);
