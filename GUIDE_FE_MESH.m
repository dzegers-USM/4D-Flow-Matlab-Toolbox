...
%     end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % disp(min(handles.nodes1)) % JSOTELO
    % disp(max(handles.nodes1)) % JSOTELO
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function pushbutton3_Callback(hObject, eventdata, handles)
    ...
    f = waitbar(0, 'Generating tetrahedral mesh ...');
    wbch = allchild(f);
    jp = wbch(1).JavaPeer;
    jp.setIndeterminate(1)
...
%     handles.nodes = [handles.nodes(:,1)-(2*handles.voxel_MR(1)),handles.nodes(:,2)-(2*handles.voxel_MR(2)),handles.nodes(:,3)-(2*handles.voxel_MR(3))];
    ...
    switch handles.answer1
        case 'Yes'
            ...
            handles.MR_PCA_FH_ori = MR_PCA_FH_old;
            handles.MR_PCA_RL_ori = MR_PCA_RL_old;
            handles.MR_PCA_AP_ori = MR_PCA_AP_old;
...
% handles    structure with handles and user data (see GUIDATA)
            ...
            if isempty(folder_name)==0

                load([folder_name,'/faces.mat']);
                load([folder_name,'/elem.mat']);
                load([folder_name,'/nodes.mat']);
                
                % disp(min(nodes)) % JSOTELO
                % disp(max(nodes)) % JSOTELO


                %%% surface-tetrahedral mesh
        
                f = waitbar(0,'Please wait: loading mesh ...');
            ...
            %         Code to run on Windows platform
                ...
                handles.faces = faces(:,1:3);
                handles.nodes = [(nodes(:,1)*1000)+(2*handles.voxel_MR(1)),(nodes(:,2)*1000)+(2*handles.voxel_MR(2)),(nodes(:,3)*1000)+(2*handles.voxel_MR(3))];
                handles.elem = elem(:,1:4);
